...
class ControllerExtensionPaymentPayginee extends Controller {
	public function index() {
        $this->load->language('payment/paygine_e');
        ...
        if ($redirect_url) {
            return $this->load->view('extension/payment/paygine_e.tpl', array(
                'button_confirm' => $this->language->get('button_confirm'),
                'action' => $redirect_url
            ));
        } else {
            return $this->load->view('extension/payment/paygine_e_error.tpl', array(
        ...
        try {
            ...
            if (($response->reason_code)) {
                ...
                if ($response->reason_code == 1){
                    $this->model_checkout_order->addOrderHistory($response->reference, 2, 'Paygine_e Success'); // Processing
                } else {
                    $this->model_checkout_order->addOrderHistory($response->reference, 16, 'Paygine_e Fail'); // Voided
                }
                die("ok");
            }
        } catch (Exception $ex) {
    ...
    public function request() {
        try {
            $this->language->load('payment/paygine_e');
            $this->load->model('checkout/order');
            if ($this->checkPaymentStatus()) {
                $this->model_checkout_order->addOrderHistory($this->request->get['reference'], 2, 'Paygine_e Success'); // Processing
                $this->response->redirect($this->url->link('checkout/success'));
            } else {
                $this->model_checkout_order->addOrderHistory($this->request->get['reference'], 16, 'Paygine_e Fail'); // Voided
    ...
    private function registerOrder($order_info) {
        ...
        if (!$this->config->get('paygine_e_test')) {
        ...
        $signature = base64_encode(md5($this->config->get('paygine_e_sector') . intval($amount * 100) . $currency . $this->config->get('paygine_e_password')));

        $fiscalPositions='';
        $KKT = $this->config->get('paygine_e_kkt');
        if ($KKT==1){
            $TAX = (strlen($this->config->get('paygine_e_tax')) > 0) ?
                intval($this->config->get('paygine_e_tax')) : 7;
            if ($TAX > 0 && $TAX < 7){
        ...
        $query = http_build_query(array(
            'sector' => $this->config->get('paygine_e_sector'),
            ...
            'currency' => $currency,
            'mode' => 1,
            'url' => HTTP_SERVER . 'index.php?route=extension/payment/paygine_e/request',
            'signature' => $signature
        ));
        ...
        if (intval($paygine_order_id) == 0) {
            ...
            $signature = base64_encode(md5($this->config->get('paygine_e_sector') . $paygine_order_id . $this->config->get('paygine_e_password')));
            return "{$paygine_url}/webapi/Epayment?sector={$this->config->get('paygine_e_sector')}&id={$paygine_order_id}&signature={$signature}";
        }
    ...
    private function checkPaymentStatus() {
        ...
        $signature = base64_encode(md5($this->config->get('paygine_e_sector') . $paygine_order_id . $paygine_operation_id . $this->config->get('paygine_e_password')));
    ...
    private function orderWasPayed($response) {
        ...
        $signature = base64_encode(md5(implode('', $tmp_response) . $this->config->get('paygine_e_password')));
